...
public class H2Initialize implements IMiddleWare {

    private static volatile H2Initialize INSTANCE;

    private H2Initialize() {
    }

    /**
     * lazy Loading, Double-Checked Locking
     */
    public static H2Initialize getInstance() {
        if (INSTANCE == null) {
            synchronized (H2Initialize.class) {
                if (INSTANCE == null) {
                    INSTANCE = new H2Initialize();
                }
            }
        }
        return INSTANCE;
    ...
    @Override
    public void run() {
        IDBConnection h2 = IDBConnection.getH2Connection();
